...
class GamingSpeechWindow:
    ...
    def read_box_text(self):
        ...
        try:
            ...
            print(f"Processed image (grayscale) saved to {processed_image_path}")

            # Use Tesseract with strict character preservation
            print("Starting OCR with Tesseract...")
            try:
                # Configure Tesseract for exact character preservation
                custom_config = r'--oem 3 --psm 6 -c tessedit_char_whitelist="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,!?@#$%^&*()[]{}<>-_=+;:\'\" "'
                text = pytesseract.image_to_string(gray_image, config=custom_config)
                print(f"OCR completed with Tesseract. Text:\n{text}")
            except Exception as e:
